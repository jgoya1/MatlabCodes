...
fid=fopen('modelo4_theeta_sweep_dat.txt','r'); %Open the file to be read
aux2=0;
for i=1:length(degrees)
    linenum=2+1004*aux2; %Jump all the numerical data
    var(i)=textscan(fid,'#"Frequency / GHz"	"E_Field (Farfield) (Spherical) (%d 0 1000)(Abs) [1] [Magnitude]"',1,'Delimiter','\n','HeaderLines',linenum-1); %Find the order in the data headers
...
for i=0:length(degrees)-1
    data(:,1)=dlmread('modelo4_theeta_sweep_dat.txt','',[3+1004*i 0 1003+1004*i 0]); %Read the frequency samples
    data(:,2)=dlmread('modelo4_theeta_sweep_dat.txt','',[3+1004*i 1 1003+1004*i 1]); %Read the electric field values
...
title('Realized Gain in H-plane');
xlabel('Frequency');
ylabel('Phi');
